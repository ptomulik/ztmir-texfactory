...
\DeclarePairedDelimiterX{\abs}[1]{|}{|}{\setargs{#1}}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[%
    ...
    \node [input] (IN) {};
    \node [block, right of=IN, node distance=2cm] (G) {$G$};
    \node [output, right of=G, node distance=2cm] (OUT) {};
    \draw [->] (IN) -- node[anchor=south]{$u_1(t)$} (G);
    \draw [->] (G) -- node[anchor=south]{$u_2(t)$} (OUT);
  \end{tikzpicture}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[%
      ...
      filt/.style = {%
        ...
        path picture={%
          \draw [black]%
          ($(path picture bounding box.west) + ( 2mm,0mm)$)%
          --%
    ...
    \node [block] (G) {$G$};
    \node [input, below of=G, node distance=2.5cm] (IN) {};
    \node [output, above of=G, node distance=2.5cm] (OUT) {};
    \node [junction, below of=G, node distance=2cm] (JI0) {};
    \node [junction, above of=G, node distance=2cm] (JO0) {};
    ...
    \draw [->] (IN) -- node[anchor=east]{$u_1(t)$} (G);
    \draw [->] (G) -- node[anchor=east]{$u_2(t)$} (OUT);
    \draw [->] (JI0) -- (JI1) -| (CI1I) -- (MII);
    \draw [->] (JI0) -- (JI1) -| (CI1Q) -- (MIQ);
    \draw [->] (JO0) -- (JO1) -| (CO1I) -- (MOI);
    \draw [->] (JO0) -- (JO1) -| (CO1Q) -- (MOQ);
    \draw [->] (UII) -- node [anchor=west]{$\sin{\omega t}$} (MII);
    \draw [->] (UOI) -- node [anchor=west]{$\sin{\omega t}$} (MOI);
    \draw [->] (UIQ) -- node [anchor=west]{$\cos{\omega t}$} (MIQ);
    \draw [->] (UOQ) -- node [anchor=west]{$\cos{\omega t}$} (MOQ);
    \draw [->] (MII) -- (FII);
    \draw [->] (MIQ) -- (FIQ);
    \draw [->] (MOI) -- (FOI);
    \draw [->] (MOQ) -- (FOQ);
